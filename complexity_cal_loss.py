...
def calculate_ppl(
    ...
    with torch.no_grad():
        for batch in tqdm(dataloader):
            ...
            logps.extend(sentence_logps.tolist())

    dir_path = os.path.dirname(save_name)
    if dir_path:
        os.makedirs(dir_path, exist_ok=True)
    with open(save_name, "w", encoding="utf-8") as f:
        json.dump(logps, f, indent=2)

    print(f"Logps loss have been saved at {save_name}.")
...
if __name__ == "__main__":
    ...
    calculate_ppl(model_name_or_path=args.model_name_or_path, save_name=args.save_name, dataset=args.dataset)
